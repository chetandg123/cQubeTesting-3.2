        ...
        for x in range(1, len(select_district.options)):
            ...
            for y in range(1, len(select_block.options)):
                ...
                for z in range(1,len(select_cluster.options)):
                    ...
                    time.sleep(5)
                    p = pwd()
                    self.filename = p.get_download_dir() + "/student_attendance_exception_"+management+"_Schools_of_cluster_"+values+ self.month + "_" + self.year+"_"+cal.get_current_date()+ ".csv"
                    print(self.filename)
                    if not os.path.isfile(self.filename):
                        print(
                            "District" + select_district.first_selected_option.text + "Block" + select_block.first_selected_option.text + "csv is not downloaded")
